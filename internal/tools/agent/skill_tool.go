...
func (t *SkillTool) Description() string {
	return "Load a SKILL.md file by its name. Use this when the user's task matches a skill description from the system prompt. Pass the skill name exactly as it appears in the <name> tag (e.g. \"k8s-debug-pod\"). Returns the skill's workflow instructions."
}
...
var LoadSkillSchema = map[string]any{
	...
	"properties": map[string]any{
		"name": map[string]any{
			"type":        "string",
			"description": "Skill name exactly as it appears in the <name> tag of <available_skills> (e.g. \"k8s-debug-pod\").",
		},
	},
