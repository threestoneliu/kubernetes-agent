...
### Requirement: 定时任务 SHALL 支持 cron 表达式和一次性时间两种触发方式

cron 表达式支持秒级精度（6 位格式），一次性任务使用 UNIX 时间戳指定执行时间。
...
### Requirement: 定时任务 SHALL 持久化到 SQLite 并在重启后恢复

所有 `enabled=1` 的任务在 Scheduler 重启后自动恢复，根据 cron 表达式重新计算 `next_run`。
...
### Requirement: 定时任务执行结果 SHALL 写入对应 session

执行时向关联 session 写入消息（`source="scheduled"`），触发 agent loop，用户在 chat 中看到执行结果。
...
### Requirement: LLM SHALL 能够通过工具创建和管理定时任务

LLM 通过 `schedule_task` / `get_scheduled_tasks` / `delete_scheduled_task` 工具操作定时任务，受 policy engine 约束。
...
### Requirement: 并发冲突 SHALL 被正确处理

如果定时触发时 session 正在被另一个 SSE 连接使用（agent loop 运行中），该次触发 skip，不重试。
...
### Requirement: UI SHALL 提供定时任务管理界面

Chat view 侧边栏提供"定时任务"tab，支持查看、创建、启用/禁用、删除任务。
