...
**K8s 凭证存储**
- From: 仓库零状态,无凭证存储
- To: 用户上传 kubeconfig 整体 AES-256-GCM 加密落 SQLite,master.key 来自环境变量或 `~/.k8s-agent/master.key`(0600)
- Reason: 凭证不出本机
...
- `k8s-credential-encryption`: kubeconfig 整体 AES-256-GCM 加密;master.key 来自 `K8S_AGENT_MASTER_KEY` 或 `~/.k8s-agent/master.key`(0600);解密仅在内存,会话结束清除
...
- 文件:`~/.k8s-agent/master.key`(0600) + `~/.k8s-agent/data.db` + `~/.k8s-agent/config.yaml`
