...
When `data-theme="dark"` is set on `<html>`, the CSS custom properties SHALL resolve to:
...
When `data-theme="light"` is set on `<html>`, the CSS custom properties SHALL resolve to:
